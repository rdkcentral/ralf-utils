...
namespace entos::ralf::dmverity
{
    ...
    {
    public:
        virtual ~IDmVerityMounter() = default;
        ...
        // -------------------------------------------------------------------------
        /*!
            Obviously defines a range of bytes within a file, it's used to
            describe the location of fs and dm-verity parts of an image file.
         ...
         */
        virtual LIBRALF_NS::Result<std::unique_ptr<LIBRALF_NS::IPackageMountImpl>>
        mount(std::string_view name, FileSystemType fsType, const std::filesystem::path &imagePath,
              ...
              LIBRALF_NS::MountFlags flags) const = 0;
        ...
        mount(std::string_view name, FileSystemType fsType, int imageFd, const std::filesystem::path &mountPoint,
              ...
              const std::vector<uint8_t> &salt, LIBRALF_NS::MountFlags flags) const = 0;
         ...
         */
        virtual LIBRALF_NS::Result<> checkImageFile(FileSystemType fsType, const std::filesystem::path &imagePath,
                                                    FileRange dataRange, FileRange hashesRange) const = 0;

        virtual LIBRALF_NS::Result<> checkImageFile(FileSystemType fsType, int imageFd, FileRange dataRange,
                                                    FileRange hashesRange) const = 0;
    };

    inline bool operator==(const IDmVerityMounter::FileRange &lhs, const IDmVerityMounter::FileRange &rhs)
    {
