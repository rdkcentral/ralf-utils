...
# a root CA and then 3 intermediate CAs with various expiry times.  It generates
...
# Working directory
WORKINGDIR="${1:-.}/../data/xmldsig/expiredchain/"
cd "$WORKINGDIR"

# Create a root CA valid from 2020 -> 2050
createRootCa "2020-01-01 01:23:45" 10958
...
TEST_DIR="test1"
  ...
  openssl pkcs12 -export \
    -name "signing" \
    -in ${TEST_DIR}/intermediate-3.crt \
...
TEST_DIR="test3"
  ...
  #openssl verify -x509_strict -no_check_time -CAfile root.crt inter-1.crt
  #openssl verify -x509_strict -no_check_time -CAfile root.crt -untrusted test1/intermediate-1.crt test1/intermediate-2.crt
  #openssl verify -x509_strict -no_check_time -CAfile root.crt -untrusted test1/intermediate-1.crt -untrusted test1/intermediate-2.crt test1/intermediate-3.crt
