...
#
# You need the faketime tool installed to run this script.
#
...
# Do all the work in the data directory
cd ../data/expired-certs

# Create a root CA valid from 2020 -> 2050
createRootCa root "2020-01-01 01:23:45" 10958

# Create expired root CA
createRootCa expired-root "2020-01-01 01:23:45" 730
...
TEST_DIR="test1"
  ...
  createIntermediateCA ${TEST_DIR}/signing        ${TEST_DIR}/intermediate-2 "2021-01-01 01:23:45" 730
  ...
  openssl verify \
    ...
    ${TEST_DIR}/signing.crt

  openssl pkcs12 -export \
    -inkey "${TEST_DIR}/signing.key" \
    -in "${TEST_DIR}/signing.crt" \
    -certfile "${TEST_DIR}/signing.crt.chain" \
    -out "${TEST_DIR}/signing.p12" \
    -name "PackageSigner" \
    -passout pass:
...
TEST_DIR="test3"
  createIntermediateCA ${TEST_DIR}/intermediate-1 root                       "2021-01-01 01:23:45" 730
  createIntermediateCA ${TEST_DIR}/intermediate-2 ${TEST_DIR}/intermediate-1 "2021-01-01 01:23:45" 730
  createIntermediateCA ${TEST_DIR}/signing        ${TEST_DIR}/intermediate-2 "2022-01-01 01:23:45" 730
...
mkdir -p test4
TEST_DIR="test4"
  # Create non-expired intermedia and signing cert
  createIntermediateCA ${TEST_DIR}/intermediate-1 expired-root               "2020-01-01 01:23:45" 10958
  createIntermediateCA ${TEST_DIR}/signing        ${TEST_DIR}/intermediate-1 "2021-01-01 01:23:45" 10958

  openssl verify \
    -x509_strict \
    -no_check_time \
    -CAfile expired-root.crt \
    -untrusted ${TEST_DIR}/intermediate-1.crt \
    ${TEST_DIR}/signing.crt

  openssl pkcs12 -export \
    -inkey "${TEST_DIR}/signing.key" \
    -in "${TEST_DIR}/signing.crt" \
    -certfile "${TEST_DIR}/intermediate-1.crt" \
    -out "${TEST_DIR}/signing.p12" \
    -name "PackageSigner" \
    -passout pass:
